...
test("normalizeQuestion preserves Question instance", () => {
  ...
  assert.strictEqual(normalizeQuestion(original), original);
});
...
test("normalizeQuestion does not mutate input", () => {
  const input = {
    id: "q1",
    text: "Capital?",
    answers: [
...
test("normalizeQuestion converts unknown correct index to answer id", () => {
  const question = normalizeQuestion({
    id: "q1",
    text: "Capital?",
    answers: ["Paris", "London"],
  ...
  assert.deepEqual(question.correct, ["a99"]);
});
